...
public final class RacCore {
    ...
    private static final Map<Integer, RacReedSolomon> RS_CACHE = new HashMap<>();

    private static RacReedSolomon rsFor(int nsym) {
        RacReedSolomon codec = RS_CACHE.get(nsym);
        if (codec == null) {
            codec = new RacReedSolomon(nsym);
    ...
    private static Options normalizeOptions(Options options) {
        ...
        if (merged.ppb < 1 || merged.ppb > RacConstants.COEFFICIENT_PAIRS.length) {
            throw new IllegalArgumentException("ppb must be in 1.." + RacConstants.COEFFICIENT_PAIRS.length);
        }
    ...
    public static int capacityBytes(int width, int height, Options options) {
        ...
        return Math.max(0, maxCodewords * (255 - o.nsym) - RacConstants.PAYLOAD_HEADER_BYTES);
    }

    /** 组装自描述头 + 载荷体。 */
    public static byte[] buildPayload(byte[] jpeg, Options options) {
        Options o = normalizeOptions(options);
        byte[] payload = new byte[RacConstants.PAYLOAD_HEADER_BYTES + jpeg.length];
        for (int i = 0; i < 4; i++) payload[i] = (byte) RacConstants.MAGIC[i];
        ...
        System.arraycopy(jpeg, 0, payload, RacConstants.PAYLOAD_HEADER_BYTES, jpeg.length);
        return payload;
    }
    ...
    private static byte[] encodeStream(byte[] payload, Options o) {
        ...
        RacReedSolomon codec = rsFor(o.nsym);
        ...
        int[] encodedBits = RacBits.bytesToBits(encoded);
    ...
    public static ExtractResult extract(ImageData imageData) {
        ...
        for (int ppb = 1; ppb <= pairCount; ppb++) {
            ...
            for (int blockIndex = 0; blockIndex < totalBlocks; blockIndex++) {
                ...
                for (int p = 0; p < ppb; p++) {
                    slotBits[base + p] = (byte) (allDeltas[deltaBase + p] >= RacConstants.READ_THRESHOLD ? 1 : 0);
                }
            }
            ...
            for (int repeat = 1; repeat <= 5; repeat++) {
                byte[] firstBits;
                if (repeat > 1) {
                    byte[] raw = RacInterleave.deinterleave(slotBits, rmax, RacConstants.CODEWORD_BITS * repeat);
                    firstBits = new byte[RacConstants.CODEWORD_BITS];
                    for (int i = 0; i < RacConstants.CODEWORD_BITS; i++) firstBits[i] = raw[i * repeat];
                } else {
                    firstBits = RacInterleave.deinterleave(slotBits, rmax, RacConstants.CODEWORD_BITS);
                }
                byte[] firstBytes = RacBits.bitsToBytes(firstBits);

                for (int nsym = 8; nsym <= 64; nsym++) {
                    RacReedSolomon codec = rsFor(nsym);
                    ...
                    if (len <= 0 || len > RacConstants.MAX_PAYLOAD_BYTES) continue;
                    int numCodewords = (RacConstants.PAYLOAD_HEADER_BYTES + len + messageBytes - 1) / messageBytes;
                    ...
                    System.arraycopy(payload, RacConstants.PAYLOAD_HEADER_BYTES, jpeg, 0, len);
    ...
    private static byte[] decodeStream(byte[] slotBits, int rmax, int repeat,
                                       int numCodewords, int messageBytes, RacReedSolomon codec) {
        byte[] allBits;
        if (repeat > 1) {
            byte[] raw = RacInterleave.deinterleave(slotBits, rmax, numCodewords * RacConstants.CODEWORD_BITS * repeat);
            allBits = new byte[numCodewords * RacConstants.CODEWORD_BITS];
            for (int j = 0; j < allBits.length; j++) allBits[j] = raw[j * repeat];
        } else {
            allBits = RacInterleave.deinterleave(slotBits, rmax, numCodewords * RacConstants.CODEWORD_BITS);
        }
        byte[] allBytes = RacBits.bitsToBytes(allBits);
